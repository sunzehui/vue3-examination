<template>
  <n-card class="exam-card" :title="exam.name + '-' + exam.id">
    <template #header-extra>
      {{ exam.for_classes.name + "-" + exam.for_classes.id }}
    </template
    >
    ...
    <template #footer>
      <NSpace justify="end">
        ...
        <n-divider vertical/>
        <n-time
            :time="localTime(exam.begin_time)"
            format="hh:mm"
            type="datetime"
        />
        -
        <n-time
            :time="localTime(exam.end_time)"
            format="hh:mm"
            type="datetime"
        />
      </NSpace>
    ...
    <template #action>
      <n-space justify="space-between">
        ...
        <n-space justify="end">
          <n-button @click="goExamRoom(exam.id)" v-role="Role.student"
          >进入考试
          </n-button
          >
          <n-button @click="goEditExamRoom(exam.id)" v-role="Role.teacher"
          >编辑考试
          </n-button
          >
        </n-space>
...
import {ApiEnterExamRoom} from '@/apis/exam-room'
...
const goExamRoom = async (id: number) => {
  ...
  if (status === TagStatus.wait) {
    ...
    const result = await ApiEnterExamRoom(id);
    if (result.statusCode === 200)
      await router.push({name: "room", params: {rid: id}});
  }
};
